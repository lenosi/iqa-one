...
class ExecutorAsyncSsh(ExecutorBase):
    ...
    This class is inspired by https://github.com/selfuryon/netdev
    """
    name = 'SSH'

    def __init__(self, host=u"", username=u"root", password=u"", port=22, device_type=u"", timeout=15, loop=None,
                 known_hosts=None, local_addr=None, client_keys=None, passphrase=None, tunnel=None, pattern=None,
                 agent_forwarding=False, agent_path=(), client_version=u"IQA", family=0, kex_algs=(),
                 encryption_algs=(), mac_algs=(), compression_algs=(), signature_algs=(), **kwargs):
        """
    ...
    async def _send_command(
        ...
        output = self._normalize_line_feeds(output)
        if strip_prompt:
    ...
    @staticmethod
    def _normalize_line_feeds(a_string):
        """Convert '\r\r\n','\r\n', '\n\r' to '\n"""
    ...
    async def _establish_connection(self):
        ...
        logger.info(
            "Host {}: Establishing connection to port {}".format(self.host, self.port)
        )
        output = ""
